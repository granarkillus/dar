...
export default function DARReport() {
  ...
  const generatePDF = () => {
    ...
    const incidents = records.filter((r) =>
      r.activity_log?.some((e) => e.activity?.includes("*"))
    ).length;

    const officerSections = Object.entries(groups)
      .map(([officer, dars]) => {
        const darRows = dars
          .map((dar) => {
            const items = [];
            ...
            const activityRows = (dar.activity_log || [])
              .filter((e) => e.activity?.trim())
              .map(
                (e) => `
              <tr>
                <td style="padding:3px 6px;border:1px solid #d1d5db;font-size:8.5pt;white-space:nowrap;">${e.from || ""}</td>
                <td style="padding:3px 6px;border:1px solid #d1d5db;font-size:8.5pt;white-space:nowrap;">${e.to || ""}</td>
                <td style="padding:3px 6px;border:1px solid #d1d5db;font-size:8.5pt;">${e.activity || ""}</td>
              </tr>`
              )
              .join("");

            return `
            <div style="margin-bottom:16px;border:1px solid #d1d5db;border-radius:3px;overflow:hidden;">
              ...
              <div style="padding:8px 12px;">
                <div style="display:flex;gap:24px;margin-bottom:6px;flex-wrap:wrap;">
                  ...
                  <div style="font-size:8pt;"><span style="font-weight:700;text-transform:uppercase;letter-spacing:0.04em;color:#6b7280;">Rest Break 1:</span> ${dar.rest_break_1_out ? `${dar.rest_break_1_out} – ${dar.rest_break_1_in}` : "—"}</div>
                ...
                <table style="width:100%;border-collapse:collapse;margin-top:6px;">
                  <thead>
                    <tr style="background:#1a1a2e;">
                      <th style="padding:4px 6px;border:1px solid #d1d5db;font-size:7.5pt;color:#fff;text-align:left;white-space:nowrap;">From</th>
                      <th style="padding:4px 6px;border:1px solid #d1d5db;font-size:7.5pt;color:#fff;text-align:left;white-space:nowrap;">To</th>
                      <th style="padding:4px 6px;border:1px solid #d1d5db;font-size:7.5pt;color:#fff;text-align:left;">Activity / Report</th>
                    </tr>
                  </thead>
                  <tbody>${activityRows}</tbody>
                </table>` : '<p style="font-size:8pt;color:#6b7280;font-style:italic;margin-top:4px;">No activity entries recorded.</p>'}
                <div style="margin-top:6px;font-size:8pt;color:#374151;border-top:1px solid #f0f0f0;padding-top:4px;">
                  <span style="font-weight:700;">Signed:</span> ${dar.signature}
                </div>
              </div>
            </div>`;
          })
        ...
        <div style="margin-bottom:24px;">
          ...
          <div style="border:1px solid #1f4e79;border-top:none;border-radius:0 0 3px 3px;padding:12px;">
            ${darRows}
          </div>
        </div>`;
      })
...
<head>
<meta charset="utf-8"/>
<title>DAR Report – ${formatDate(startDate)} to ${formatDate(endDate)}</title>
<style>
...
<body>
  <div style="background:#1f4e79;padding:14px 20px;display:flex;justify-content:space-between;align-items:center;margin-bottom:0;">
    <div>
      <div style="color:#fff;font-size:13pt;font-weight:800;letter-spacing:0.06em;text-transform:uppercase;">Allied<span style="font-weight:300;">Universal</span><sup style="font-size:6pt;font-weight:300;">™</sup></div>
      <div style="color:rgba(255,255,255,0.6);font-size:7.5pt;margin-top:1px;">Security Services</div>
    </div>
    <div style="text-align:right;">
      <div style="color:#fff;font-size:12pt;font-weight:700;">Daily Activity Report — Compiled</div>
      <div style="color:rgba(255,255,255,0.75);font-size:8.5pt;">Washington University · Saint Louis</div>
    </div>
  </div>
  ...
  <div style="background:#f4f6f9;border:1px solid #d1d5db;border-top:none;padding:8px 20px;display:flex;gap:32px;margin-bottom:20px;">
    ...
    ${officerFilter ? `<div style="font-size:8.5pt;"><span style="font-weight:700;text-transform:uppercase;letter-spacing:0.04em;color:#6b7280;">Filter:</span> ${officerFilter}</div>` : ""}
  </div>

  ${officerSections}

  <div style="border-top:1px solid #d1d5db;margin-top:24px;padding-top:8px;font-size:7.5pt;color:#6b7280;text-align:center;">
    Allied Universal Security Services · Washington University · Generated ${new Date().toLocaleDateString("en-US", { year: "numeric", month: "long", day: "numeric" })} · Please keep all completed forms on file for audit purposes.
  </div>
</body>
</html>`;
    ...
    <div style={{ minHeight: "100vh", background: SOFT_BG, fontFamily: "'Helvetica Neue', Helvetica, Arial, sans-serif", padding: "2rem 1rem" }}>
      <div style={{ maxWidth: 800, margin: "0 auto" }}>
        ...
        <div style={{ background: WHITE, border: `1px solid ${BORDER}`, borderTop: "none", padding: "1.5rem 2rem", marginBottom: "1.5rem" }}>
          <div style={{ fontSize: "0.72rem", fontWeight: 700, color: "#374151", textTransform: "uppercase", letterSpacing: "0.06em", marginBottom: "1rem" }}>
            Search Parameters
          </div>
          <div style={{ display: "flex", gap: "1rem", flexWrap: "wrap", alignItems: "flex-end" }}>
            ...
            <div>
              <button
                ...
                style={{
                  ...btnStyle(startDate && endDate && !loading ? NAVY : "#9ca3af"),
                  cursor: startDate && endDate && !loading ? "pointer" : "not-allowed",
          ...
          {error && (
            <div style={{ marginTop: "0.75rem", background: "#fef2f2", border: "1px solid #fca5a5", borderRadius: 4, padding: "0.6rem 1rem", fontSize: "0.82rem", color: "#b91c1c" }}>
              {error}
            </div>
          )}
        </div>
              ...
              <>
                ...
                <div style={{ background: DARK, padding: "0.75rem 2rem", display: "flex", gap: "2rem", flexWrap: "wrap", borderRadius: "4px 4px 0 0" }}>
                  {[
                    ...
                    ["Officers", Object.keys(groups).length],
                    ["Incidents (*)", records.filter((r) => r.activity_log?.some((e) => e.activity?.includes("*"))).length],
                  ].map(([label, val]) => (
                ...
                {Object.entries(groups).map(([officer, dars]) => (
                  <div key={officer} style={{ background: WHITE, border: `1px solid ${BORDER}`, borderTop: "none", marginBottom: 0 }}>
                ...
                <div style={{ background: WHITE, border: `1px solid ${BORDER}`, borderTop: "none", padding: "1rem 2rem", borderRadius: "0 0 4px 4px" }}>
                  <button onClick={generatePDF} style={{ ...btnStyle(NAVY), width: "100%", display: "flex", alignItems: "center", justifyContent: "center", gap: 8 }}>
                    Generate PDF Report
                  </button>
                </div>
              </>
...
function btnStyle(bg: string): React.CSSProperties {
  return {
    background: bg, color: "#ffffff", border: "none", borderRadius: 4,
